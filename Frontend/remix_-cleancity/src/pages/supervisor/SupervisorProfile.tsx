...
const DEFAULT_PROFILE: SupervisorProfileData = {
...
export const SupervisorProfile: React.FC = () => {
  ...
  const dashboardPath = "/supervisor/dashboard";
  const [profile, setProfile] = useState<SupervisorProfileData>(DEFAULT_PROFILE);
...
role: data.role || "Zone Supervisor",
  ...
  phone: data.mobile_number || DEFAULT_PROFILE.phone,

 lastLogin: data.created_at || DEFAULT_PROFILE.lastLogin,
 status: data.status || "Active",
};

      setProfile(supervisorProfile);
      setEditForm(supervisorProfile);
    } catch (err) {
...
>
  ...
  Manage your supervisor credentials and account information.
</p>
          </div>
          <button 
    onClick={() => navigate(dashboardPath)}
                ...
                {/* Supervisor Role */}
                                <div className="space-y-1 text-left">
                                  ...
                                  {isEditing ? (
                                    ...
                                    >
                                     <option value="Zone Supervisor">
  Zone Supervisor
</option>

<option value="Senior Zone Supervisor">
